    ...
    elseif has('unix') && s:IsLanguageInterpreting(l:cur_filetype)
        ...
        if l:async
            ...
            exec (l:show_result_after_run ? 'silent ' : '').
                        \ 'SplitTerminal ('.l:compile_cmd.' '.l:compile_args.'; '.
                        \ 'echo '.s:GetShellLastExitCodeVariable().' >'.
                        \ l:exit_code_tempfile.') '.s:GetShellPipe(1).' '.
                        \ s:run_result_tempfile

            exec 'silent !(echo 0 > ' . l:exit_code_tempfile . ')'
            exec 'silent !(touch ' . s:run_result_tempfile . ')'
        ...
        if v:shell_error != 0
            ...
            if g:called_by_compile_run != 0
                "Hide the message
                call feedkeys("\<CR>")
            else
                echohl MoreMsg | echo 'Compile successful' | echohl none
            endif
        endif
...
function! s:Run(async) " {{{1
    ...
    if l:async
        ...
        try
            exec (l:show_result_after_run ? 'silent ' : '').'SplitTerminal '.l:run_cmd
        catch
